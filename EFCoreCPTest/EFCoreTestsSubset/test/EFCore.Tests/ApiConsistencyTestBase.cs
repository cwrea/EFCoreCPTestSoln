...
namespace Microsoft.EntityFrameworkCore
{
    public abstract class ApiConsistencyTestBase
    {
        ...
        // XAMARIN BUG: Fails on Xamarin.iOS. Why is that?
        [Fact]
            ...
            var parametersMissingAttribute
                = (from type in GetAllTypes(TargetAssembly.GetTypes())
                   ...
                   where (method.DeclaringType == type)
                         ...
                         && (method.Name != "OnConfiguring") // CHANGED for Xamarin testing: WAS: nameof(DbContext.OnConfiguring)
                         && (method.Name != "OnModelCreating") // CHANGED for Xamarin testing: WAS: nameof(DbContext.OnModelCreating)
