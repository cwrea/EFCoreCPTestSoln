    ...
    public class SqliteTestStore : RelationalTestStore
    {
        ...
        public static SqliteTestStore CreateScratch(bool sharedCache = false)
        {
            ...
            while (File.Exists(name + ".db")
                   || (!string.IsNullOrEmpty(BaseDirectory) && File.Exists(Path.Combine(BaseDirectory, name + ".db")))); // CHANGED for Xamarin testing. Added guard. TODO: Is BaseDirectory appropriate on Android?
